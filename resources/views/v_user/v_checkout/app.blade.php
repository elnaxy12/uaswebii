        ...
        <form action="{{ route('checkout.process') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="address" rows="3"
                        class="border px-3 py-2 rounded w-full" required>{{ auth()->user()->address }}</textarea>
            </div>
            ...
            <button type="submit"
                class="inline-block border bg-black text-white px-6 py-2 rounded hover:bg-white hover:border-black hover:text-black focus:bg-white border-1 focus:text-black focus:border focus:border-black cursor-pointer text-sm">
                Confirm & Place Order
            </button>
        </form>
    </div>
