                    ...
                    <tbody>
                        @foreach($orders as $order)
                            <tr class="text-center border-b-gray-200 border-b-1 text-xs">
                                ...
                                <td class="p-2 flex justify-center flex-col md:flex-row gap-2">
                                    ...
                                    @if($order->status === 'pending')
                                        <form action="{{ route('order.cancel', $order->id) }}" method="POST">
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="bg-black text-white text-sm px-6 py-2 border-white border-1 hover:bg-red-100 hover:text-red-500 hover:border-black focus:bg-red-100 focus:text-red-500 focus:border-black cursor-pointer">Cancel</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
